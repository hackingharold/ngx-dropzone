...
export class AcceptService {
  ...
  accepts(fileValue: FileInputValue, accept: string): boolean {
    ...
    return fileList.every((file) =>
      this.isAcceptedByExtension(file, acceptedExtensions) ||
      this.isAcceptedByMimeType(file, acceptedMimeTypes));
  }
  ...
  private isValidMimeType(type: string): boolean {
    ...
    return Array.from(safeType).every(
      (char, i) => this.isValidToken(char) || i === slashPos);
  }
