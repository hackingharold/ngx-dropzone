...
describe('FileInputValidators', () => {
  ...
  const getFile = (size: number) => new File([Math.random().toString(16).slice(0, size)], `${Date.now()}.rnd`);

  it('should validate min size', () => {
    const control = new FormControl<FileInputValue>(getFile(16), [FileInputValidators.minSize(8)]);
  ...
  it('should validate max size', () => {
    const control = new FormControl<FileInputValue>(getFile(4), [FileInputValidators.maxSize(8)]);
  ...
  it('should pass null check', () => {
    const control1 = new FormControl<FileInputValue>(null, [FileInputValidators.minSize(8)]);
    const control2 = new FormControl<FileInputValue>(null, [FileInputValidators.maxSize(8)]);
